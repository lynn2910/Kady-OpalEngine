...
use crate::coolors::colors::ColorCount;
...
pub fn get_most_freq(set: &HashSet<ColorCount>, n: usize) -> Vec<&ColorCount> {
    ...
    for c in set.iter() {
        if let Some(v) = heap.peek() { if v.0.1.measure_distance(c) < i32::abs(250) {
            continue
        } }
...
pub fn get_colors_from(img: &DynamicImage) -> HashSet<ColorCount> {
        ...
        let handle = thread::spawn(move || {
            for i in &tmp.lock().unwrap()[start..finish] {
                ...
                if !col_set.contains(&c) {
                    col_set.insert(c);
                } else if let Some(mut v) = col_set.take(&c) {
                    v.increment_count();
                    col_set.insert(v);
                }
            }
    ...
    let res  = colors.lock().unwrap().clone();
    res
}
